...
class MovieStore: MovieService {
    ...
    private let apiKey = "705285b1208ed4a6bd0df4421301a73e"
    ...
    func fetchMovie(id: Int, completion: @escaping (Result<MovieResponse, MovieError>) -> ()) {
        ...
        self.loadURLandDecode(url: url, params: [
            "append_url_response":"video,credits"
    ...
    private func loadURLandDecode<D: Decodable>(url:URL, params: [String: String]? = nil, completion: @escaping (Result<D, MovieError>) -> ()) {
        guard var urlComponenets = URLComponents(url: url, resolvingAgainstBaseURL: false) else {
            completion(.failure(.invalidEndpoint))
            return
        }
